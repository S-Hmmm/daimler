...
HISTOGRAMS_CHECKPOINT_LOCATION = "spark.checkpoint.location.histograms"

STORAGE_ACCOUNT = "spark.storage.account.name"
STORAGE_ACCOUNT_KEY = "spark.storage.account.key"

HISTOGRAMS_SAVE_PATH = "spark.histograms.save.path"


def run():
    spark = SparkSession.builder.appName('adas_histograms').getOrCreate()
    ...
    histograms_checkpoint_location = conf.get(HISTOGRAMS_CHECKPOINT_LOCATION)
    histograms_save_path = conf.get(HISTOGRAMS_SAVE_PATH)
    ...
    # histograms data save
    histograms_df = df.filter(df.eventtype == "histograms").drop('date')
    histograms = (
        histograms_df.coalesce(1).writeStream
        .partitionBy('year', 'month', 'day')
        .format('parquet')
        .option('maxRecordsPerFile', 1)
        .option('failOnDataLoss', False)
        .option('checkpointLocation', histograms_checkpoint_location)
        .option('path', histograms_save_path)
        .start()
    )

    histograms.awaitTermination()
